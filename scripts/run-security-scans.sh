...
docker build --target production --tag "$image_name" "$project_directory"

docker run --rm \
  --volume "$project_directory:/repo:ro" \
  "$gitleaks_image" \
...
docker run --rm \
  ...
  --skip-dirs dist \
  /workspace |
  node "$project_directory/scripts/validate-license-report.mjs"
...
docker run --rm \
  ...
  --severity HIGH,CRITICAL \
  "$image_name" |
  node "$project_directory/scripts/validate-trivy-report.mjs"
